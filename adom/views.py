    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if action == None:
                ...
                # music_stream.write('midi', midi_filename)
                midi_file_path = r'C:\Users\KOFI ADUKPO\Desktop\code\aseda\static\output.mid'
                music_stream.write('midi', fp=midi_file_path)
                # music_stream.write('midi', fp='output.mid')
                # os.system('fluidsynth -ni sound_font.sf2 output.mid -F output.wav')
            data = json.loads(request.body.decode('utf-8'))
            values = data.get('values')
        ...
        timer = 1
        main = []
        for y in modified_data:
            y = [str(item).replace('-', 'b') for item in y]
            print(y)
            p = y
            p_annex = []
            note = []
        ...
        return render(request, 'home.html', {'timer':timer, 'chords':chrd, 'main':main, 'random_word': random_word})
        # return JsonResponse({'error': 'Invalid request method'}, status=405)
...
def play_midi(request):
    # Assuming you have a MIDI file named 'example.mid' in your media folder
    midi_file_path = "C:\\Users\\KOFI ADUKPO\\Desktop\\code\\aseda\\static\\output.mid"
    ...
    return render(request, 'music_player.html', {'midi_messages': midi_messages})
